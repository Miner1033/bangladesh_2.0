...
class _EditDocumentPageState extends State<EditDocumentPage> {
  ...
  Future<String> uploadToCloudinary(XFile image) async {
    try {
      ...
      String uploadPreset = "flutter_uploads"; // Replace with your Upload Preset
  ...
  void editDocument() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(widget.documentId).update({
        ...
        'photos': finalPhotoUrls.isNotEmpty ? finalPhotoUrls : widget.document['photos'] ?? [],
    ...
    return DropdownButtonFormField(
      ...
      items: categories
          .map((cat) => DropdownMenuItem<String>(
              value: cat, child: Text(cat, style: const TextStyle(fontSize: 16))))
          .toList(),
      onChanged: (value) {
        setState(() {
