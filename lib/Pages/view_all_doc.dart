...
class _ViewDocumentsPageState extends State<ViewDocumentsPage> {
  ...
  void initState() {
    super.initState();
    fetchDocuments(); // Fetch data when the page loads
  }

  Future<void> fetchDocuments() async {
    try {
      QuerySnapshot snapshot =
          await _firestore.collection('users').get(); // Get all users
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              ...
              onPressed: () {
                Navigator.of(context).pop(); // Close the dialog
                deleteDocument(docId); // Proceed with deletion
              },
            ),
          ],
        );
...
// 🔹 Modify deleteDocument to call the dialog
  void deleteDocument(String docId) async {
    try {
      await _firestore.collection('users').doc(docId).delete();
      fetchDocuments(); // Refresh the list after deletion
  ...
  Widget buildImage(String? imagePath) {
    ...
    try {
      ...
      return CircleAvatar(
        radius: 30,
        backgroundImage: NetworkImage(imagePath), // Assume it's a URL
